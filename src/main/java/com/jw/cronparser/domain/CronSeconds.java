...
public class CronSeconds implements CronToken {
    ...
    private static CronSeconds parseElement(String expression) {
        ...
        if (matcherAll.matches()) {
            return new CronSeconds(Integer.parseInt(matcherAll.group(1)), Integer.parseInt(matcherAll.group(3)), Integer.parseInt(matcherAll.group(2)));
        } else if (matcherEvery.matches()) {
            return new CronSeconds(Integer.parseInt(matcherEvery.group(1)), Integer.parseInt(matcherEvery.group(2)), null);
        } else if (matcherRange.matches()) {
            return new CronSeconds(Integer.parseInt(matcherRange.group(1)), null, Integer.parseInt(matcherRange.group(2)));
        } else if (matcherSimple.matches()) {
            return new CronSeconds(Integer.parseInt(matcherSimple.group(1)), null, null);
        } else {
            throw new IllegalArgumentException("Wrong Cron format for seconds: " + expression);
        }
    }

    public CronSeconds(Integer start, Integer every, Integer end) {
        assert start != null;
